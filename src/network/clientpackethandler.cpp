...
#include "client/safeluanti_client.h"
...
void Client::handleCommand_ChatMessage(NetworkPacket *pkt)
{
	...
	*pkt >> chatMessage->sender >> chatMessage->message >> timestamp;
	...
	// Convert to UTF-8 for filtering
	std::string sender_utf8 = wide_to_utf8(chatMessage->sender);
	std::string message_utf8 = wide_to_utf8(chatMessage->message);

	// 1. If sender already blocked → suppress immediately
	if (m_blocked_users.count(sender_utf8)) {
		delete chatMessage;
		return;
	}

	// 2. Send to SafeLuanti backend for inspection
	...
	// 3. Enforce decision
	if (decision == BLOCK) {
		// Censor message with *** and block sender
		m_blocked_users.insert(sender_utf8);
		chatMessage->message = L"***";
		// Log the chat message (censored)
		actionstream << "CHAT (BLOCKED): " << sender_utf8 << ": ***" << std::endl;
	} else if (decision == FLAG) {
		// Report user and hide message completely
		m_blocked_users.insert(sender_utf8);
		actionstream << "CHAT (FLAGGED): " << sender_utf8 << ": " << message_utf8 << std::endl;
		
		// Show system message to player reporting the flagged user
		std::wstring report_msg = utf8_to_wide("User " + sender_utf8 + " has been reported for inappropriate content.");
		pushToChatQueue(new ChatMessage(CHATMESSAGE_TYPE_SYSTEM, report_msg));
